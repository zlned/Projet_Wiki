...
--
-- Client :  127.0.0.1
-- Généré le :  Jeu 25 Décembre 2014 à 20:15
...
--
-- Doublure de structure pour la vue `vue_equipe`
--
CREATE TABLE IF NOT EXISTS `vue_equipe` (
`equipe` varchar(255)
,`country` varchar(255)
,`trophee` varchar(255)
,`stade` varchar(255)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vue_photo_players` AS select `j`.`nom_joueur` AS `joueur`,`e`.`id_equipe` AS `ide`,`j`.`id_equipe` AS `jie`,`j`.`id_joueur` AS `jij` from (`equipe` `e` join `joueur` `j`) where ((`e`.`id_equipe` = `j`.`id_equipe`) and (`e`.`id_equipe` = 1));
